...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 500508708) return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
...
var callback = () => api.sendMessage({body:`🌸 𝗖𝗵𝘂́𝗰 𝗺𝘂̛̀𝗻𝗴 𝗯𝗮̣𝗻 𝗻𝗵𝗲́\n🌸 𝗡𝗲̂́𝘂 𝗯𝗮̣𝗻 𝘁𝗼̉ 𝘁𝗶̀𝗻𝗵 𝗰𝗿𝘂𝘀𝗵 𝘁𝗵𝗶̀ ${tile}% 𝗹𝗮̀ 𝗯𝗮̣𝗻 𝘀𝗲̃ 𝗰𝗼́ 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝘆𝗲̂𝘂 ⚜`, attachment: fs.createReadStream(__dirname + "/cache/5.gif")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.gif")); 
    ...
    var dataa = event.messageReply.attachments[0].url || args.join(" ");
  if(dataa['type'] !== 'photo') return
  const ress = await axios.get(`https://api.phamvandien.xyz/imgur?link=${encodeURIComponent(dataa.url)}`)
	const link = ress.data.uploaded.image
  var res = await axios.get(`https://manhict.tech/api/checknude?key=GmxiSuVs&url=${link}`);
