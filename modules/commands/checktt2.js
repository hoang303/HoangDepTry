...
module.exports.run = async function ({ args, api, event }) {
  var key = "free_46765ec7-0cd7-4467-8bdf-6a5ea4eae655"
    ...
    if (args[0] == "all") {
  let os = (await axios.get(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/check_rank_img?xp=0&tid=${threadID}&uid=${senderID}&api_key=${key}`)).data
arr=os.cRank.xp
...
return rank
        ...
        if (mention[0]) {
            ...
            let os = (await axios.get(encodeURL(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/check_rank_img?xp=${exp[rank].exp}&tid=${threadID}&uid=${senderID}&api_key=${key}`))).data
          if (os.status == false) return api.sendMessage(os.msg, threadID, messageID)
       return api.sendMessage({body: `=== ${exp[rank].name} ===\n\n➣ Số tin nhắn: ${exp[rank].exp}\n➣ Tỉ lệ tương tác: ${(exp[rank].exp/count*100).toFixed(0)}%\n\n➣ Xếp hạng:\n • Mức rank: ${os.cRank.rank}\n • Top: ${rank + 1}\n\n➣ Ngày gia nhập:\n • ${check_joinTime[senderID+threadID].gio}\n • ${check_joinTime[senderID+threadID].burh}`, attachment: await DownLoad(os.cRank.url)}, threadID, messageID);
        }
    else {
        var exp = [], count = 0
        for(const user of data.data) {
        ...
        const os = (await axios.get(encodeURI(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/check_rank_img?xp=${exp[rank].exp}&tid=${threadID}&uid=${senderID}&api_key=${key}`))).data
      if (os.status == false) return api.sendMessage(os.msg, threadID, messageID)
       return api.sendMessage({body: `=== ${exp[rank].name} ===\n\n➣ Số tin nhắn: ${exp[rank].exp}\n➣ Tỉ lệ tương tác: ${(exp[rank].exp/count*100).toFixed(0)}%\n\n➣ Xếp hạng:\n • Mức rank: ${os.cRank.rank}\n • Top: ${rank + 1}\n\n➣ Ngày gia nhập:\n • ${check_joinTime[senderID+threadID].gio}\n • ${check_joinTime[senderID+threadID].burh}`, attachment: await DownLoad(os.cRank.url)}, threadID, messageID);
    }
 }
