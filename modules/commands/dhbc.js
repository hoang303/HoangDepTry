const coinsup = 2000 //thay số coins được nhận khi đoán trúng
const coinsdown = 1000 //thay số coins bị mất khi yêu cầu gợi ý
...
module.exports.handleReply = async function ({
    ...
    case "choosee": {
        ...
        case "2": {
            ...
            const res = await axios.get(`https://api-vip.miraiofficials123.repl.co/game/dhbc2`);
            const length1 = res.data.dhbc.length
            const dataGame = res.data.dhbc[Math.floor(Math.random() * length1)]
        ...
        case "1": {
            api.unsendMessage(handleReply.messageID);
            const res = await axios.get(`https://raw.githubusercontent.com/Tranthanhduong565/apikey-DATA/main/datadhbc2.json`);
            const length2 = res.data.dhbc2.length
            const dataGame = res.data.dhbc2[Math.floor(Math.random() * length2)]
            const tukhoadung = dataGame.tukhoa;
            const suggestions = dataGame.suggestions
            const fs = global.nodemodule["fs-extra"];
            const sokitu = dataGame.sokitu;
...
module.exports.run = async function ({
    ...
    if (args[0] == '1') {
    api.unsendMessage(handleReply.messageID);
            const res = await axios.get(`https://API-VIP.miraiofficials123.repl.co/game/dhbc1`);
            const length2 = res.data.dhbc2.length
            const dataGame = res.data.dhbc2[Math.floor(Math.random() * length2)]
            const tukhoadung = dataGame.tukhoa;
            const suggestions = dataGame.suggestions
