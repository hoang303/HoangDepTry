...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {  
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != 100080431916488) return api.sendMessage(`𝐐𝐮𝐲𝐞̂̀𝐧 𝐥𝐨̂̀𝐧 𝐛𝐢𝐞̂𝐧 𝐠𝐢𝐨̛́𝐢 🎀`, event.threadID, event.messageID)
       ...
       const { resolve } = require("path");
        ...
        const { adminbox } = database;   
          if (permssion < 1) return api.sendMessage("𝗠𝗢𝗗𝗘 - Quyền nồn biên giới ", threadID, messageID);
   ...
   case 'ndhonly':
        case '-ndh': {
            //---> CODE ADMIN ONLY<---//
   if (permssion < 2) return api.sendMessage("𝗠𝗢𝗗𝗘 - Quyền nồn biên giới ", threadID, messageID);       
